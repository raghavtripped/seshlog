...
// --- Helper Functions (moved outside the component to prevent re-declaration) ---
...
const SessionFormComponent = ({ 
  ...
  const getDefaultState = useCallback(() => {
    ...
    return {
      ...
      customServingSize: initialSession && initialSession.liquor_serving_size === 'Custom' ? (initialSession.quantity / 1) : 0, // Simplified logic
      notes: initialSession?.notes || '',
      rating: initialSession?.rating || 3,
      sessionDate: initialSession ? new Date(initialSession.session_date).toISOString().slice(0, 16) : now.toISOString().slice(0, 16),
    };
  }, [initialSession, sessionTypes]);

  const [formState, setFormState] = useState(getDefaultState());

  // Effect to sync form state if the session being edited changes
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (category === 'liquor') {
      ...
      if (formState.liquorServingSize === 'Custom') {
        ...
        // When custom, quantity can represent the ml value itself if servings is 1.
        // Or you can adjust logic. For now, let's keep it simple.
      }
    }
  ...
  }, [category, formState, servingSizes]);
    ...
    <div className="p-4 md:p-6 bg-gray-50 dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
          <Button type="submit" disabled={isSubmitting} className={`w-full md:flex-1 h-12 text-base font-semibold bg-gradient-to-r ${categoryVisuals.gradient} text-white rounded-lg`}>
...
// --- Quantity Control Sub-component (for reusability and clean code) ---
const QuantityControl = ({ label, value, onChange, min = 1, max = 50, step = 1 }: {
  label: string;
  value: number;
  ...
  <div className="space-y-2">
    ...
    <div className="flex items-center gap-2">
      ...
      }} className="h-12 text-center text-lg font-bold rounded-lg bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600" />
