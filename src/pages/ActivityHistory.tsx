...
import { ArrowLeft, Activity, Trash2, Calendar, Clock, MapPin, Flame } from 'lucide-react';
...
const ActivityHistory = () => {
  ...
  const { entries, isLoading, deleteEntry } = useActivityEntries();
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      toast({
        title: "Deleted",
        description: "Activity entry deleted successfully",
      });
    } catch (error) {
      toast({
        title: "Error",
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center mb-6">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="heading-lg">Activity History</h1>
          ...
          {entries.length === 0 ? (
            <Card>
              <CardContent className="p-6 text-center">
                ...
                <p className="text-muted-foreground">No activity entries yet</p>
                <Button
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-4">
                      <div className="text-2xl">
                        {getActivityEmoji(entry.activity_type)}
                      </div>
                      <div className="flex-1">
                        <div className="flex items-center space-x-2 mb-2">
                          <span className="font-semibold capitalize">{entry.activity_type}</span>
                        ...
                        <div className="flex items-center space-x-4 mb-2">
                          ...
                          {entry.distance_km && (
                            <div className="flex items-center space-x-1">
                              ...
                              <span className="text-sm">{entry.distance_km}km</span>
                            </div>
                          )}
                          {entry.calories_burned && (
                            <div className="flex items-center space-x-1">
                              <Flame className="w-3 h-3 text-orange-500" />
                              <span className="text-sm">{entry.calories_burned} cal</span>
                            </div>
                          )}
                        </div>
