...
import { ArrowLeft, Heart, Trash2, Calendar, Zap, AlertTriangle } from 'lucide-react';
...
const MoodHistory = () => {
  ...
  const { entries, isLoading, deleteEntry } = useMoodEntries();
  ...
  const getMoodEmoji = (type: string, level: number) => {
    if (level <= 3) return '😔';
    if (level <= 6) return '😐';
    if (level <= 8) return '🙂';
    return '😊';
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center mb-6">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="heading-lg">Mood History</h1>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-4">
                      <div className="text-3xl">
                        {getMoodEmoji(entry.mood_type, entry.mood_level)}
                      </div>
                      <div className="flex-1">
                        <div className="flex items-center space-x-2 mb-2">
                          <span className="font-semibold capitalize">{entry.mood_type}</span>
                          <span className={`font-medium ${getLevelColor(entry.mood_level)}`}>
                            {entry.mood_level}/10
                          </span>
                        </div>
                        <div className="grid grid-cols-2 gap-4 mb-2">
                          ...
                          <div className="flex items-center space-x-1">
                            ...
                            <span className="text-sm">Stress: {entry.stress_level}/10</span>
                          </div>
                        </div>
                        {entry.triggers && (
